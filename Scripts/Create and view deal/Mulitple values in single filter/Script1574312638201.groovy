...
WebElement Counterparty_value1 = driver.findElement(By.xpath("//div[contains(text(),'3PHASES')]"))
actions.click(Counterparty_value1).perform()
...
for (def row = 2; row <= rows_count; row++) {
	...
	String ExpectedCounterparty1 ="3PHASES"
	
wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='objbox']//tr[2]//td[8]")))
...
WebUI.comment(ActualCounterparty)
	...
	if ( ActualCounterparty .equals (ExpectedCounterparty) ||  ActualCounterparty .equals (ExpectedCounterparty1)   ) {
		WebUI.comment('Pass')
	} else {
		WebUI.comment('Fail')
