...
//def sheet = 'audit'
row_count = (findTestData('change Password').getRowNumbers())

for (def row = 1; row <= row_count; row++) {
WebUI.callTestCase(findTestCase('Password Change/OldUserPasswordChange'), [('row') : row], FailureHandling.CONTINUE_ON_FAILURE)
}